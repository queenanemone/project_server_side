...
async function main () {
    ...
    try {
        ...
        console.log('Submit WithDraw transaction.');

        const withDrawResponse = await contract.submitTransaction('withDraw', 'KoreaBioLab', 'asdqw123qawsed', '2022-10-30');

        // process response
        console.log('Process WithDraw transaction response.');

        let consent = BioConsent.fromBuffer(withDrawResponse);
        
        console.log(`${consent.User_UID} consent : ${consent.TimeStamp} successfully WithDraw valid `);
